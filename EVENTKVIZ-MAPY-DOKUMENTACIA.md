...
pdf_filename: EVENTKVIZ-MAPY-DOKUMENTACIA-v0.2.pdf
---
...
- `sk-chko.geojson` — **14 chránených krajinných oblastí SR** (od v1.18.23). Zdroj OSM Overpass (relations s `boundary=protected_area` + `protection_title=Chránená krajinná oblasť`), outer ways assembled, simplify N=3, round 5 dp. Niektoré CHKO sú MultiPolygon (Biele Karpaty, Dunajské luhy, Kysuce, Malé Karpaty, Záhorie). Pripravený `mapquiz_template` „Chránené krajinné oblasti SR" (post 2141). Tool: `tools/build-sk-chko.py`. (Súbor ~414 KB.)
- `world-mountains.geojson` — **16 pohorí sveta** (od v1.21.0, dataset „Pohoria sveta", región `world`, geometry polygon, singular „pohorie"). Reálne obrysy z **Natural Earth 10m** `ne_10m_geography_regions_polys` (public domain), RDP simplify tol 0.02° + round 4 dp → 1661 vrcholov (z 2769): Himálaj, Karakoram, Hindúkuš, Pamír, Ťan-šan, Altaj, Andy, Skalnaté vrchy, Apalačské pohorie, Sierra Nevada, Aljašské pohorie, Atlas, Južné Alpy (NZ), Alpy, Kaukaz, Sierra Madre. Sierra Madre = zlúčené 3 NE časti (Occidental + Oriental + del Sur) → MultiPolygon; Ťan-šan, Andy, Južné Alpy tiež MultiPolygon (L.geoJSON ich renderuje natívne). Tool: `tools/build-world-mountains.py`. (Súbor ~33 KB.) Pripravený `mapquiz_template` „Pohoria sveta" (post 2145, region world, area, všetkých 16 v poole) — viditeľný v admin menu 🗺️ Mapové šablóny.
- `sk-mountains.geojson` — **31 pohorí SR** (od v1.18.18). Pôvodných 16 (Biele Karpaty, Branisko, Levočské vrchy, Malá Fatra, Malé Karpaty, Nízke Tatry, Poľana, Slanské vrchy, Slovenský kras, Strážovské vrchy, Tribeč, Veľká Fatra, Vihorlat, Vysoké Tatry, Západné Tatry, Štiavnické vrchy) z OSM. V1.18.18 doplnené **15 ďalších** z OSM Overpass (relation s `natural=mountain_range` alebo `boundary=geomorphological-unit`, outer ways assembled do polygon ringov, simplify N=3, round 5 dp): **Tier 1** — Považský Inovec, Vtáčnik, Kremnické vrchy, Volovské vrchy, Pieniny, Čergov, Javorníky. **Tier 2** — Bukovské vrchy, Vihorlatské vrchy, Spišská Magura, Krupinská planina, Cerová vrchovina, Čierna hora, Kysucké Beskydy, Žiar. Tool: `tools/extend-sk-mountains-tier1-2.py`. (Súbor ~229 KB.)
